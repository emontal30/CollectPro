...
/* ========== PWA Install Modal (Disabled) ========== */
function showInstallPrompt() {
  // This function is intentionally left empty to disable the PWA install prompt.
  console.log('PWA install prompt is disabled.');
}

// Show install modal automatically when eligible
// Disabled: Remove PWA install prompt per request
// window.addEventListener('beforeinstallprompt', (e) => {
//   e.preventDefault();
//   window.deferredPrompt = e;
//   // Show the attractive install modal
//   // showInstallPrompt();
// });
/* ========== Dark Mode ========== */
...
document.addEventListener("DOMContentLoaded", () => {
  // إذا كنا في صفحات التطبيق (ولسنا في login) تأكد من تسجيل الدخول
  const page = window.location.pathname.split('/').pop();
  const protectedPages = ['index.html', 'harvest.html', 'archive.html', '', null, undefined];
  if (protectedPages.includes(page) || !page) {
    if (window.auth && page !== 'login.html') {
