  ...
  actions: {
    ...
    async saveRowsToStorage(skipCloud = false) {
      ...
      try {
        ...
        localStorage.setItem('harvest_last_local_update', Date.now().toString());
    ...
    async syncFromCloud() {
      ...
      if (data) {
        ...
        if (localEmpty) {
          // If local is empty, always accept cloud
          this._applyCloudData(data);
        } else {
          // If local has data, check timestamps
          const localLastUpdate = parseInt(localStorage.getItem('harvest_last_local_update') || '0');
          const cloudLastUpdate = new Date(data.updated_at).getTime();

          // Only apply cloud if it's SIGNIFICANTLY newer (e.g., > 2 seconds to account for clock drift)
          // AND we are not the ones who just updated it (checked by last_updated_by in some cases, but timestamp is safer here)
          if (cloudLastUpdate > localLastUpdate + 2000) {
            logger.info('☁️ Cloud data is newer (' + new Date(cloudLastUpdate).toLocaleTimeString() + ') than local (' + new Date(localLastUpdate).toLocaleTimeString() + ') - Apply Sync');
            this._applyCloudData(data);
          } else {
            logger.info('🔒 Local data is newer or equal - Keeping local');
          }
        }
      }
    },
    ...
    _applyCloudData(data) {
      ...
      // Update local storage to match (without triggering another sync)
      const rowsToSave = safeDeepClone(this.rows);
      localforage.setItem(HARVEST_ROWS_KEY, rowsToSave);
      // Update our timestamp to match cloud's time so we are in sync
      localStorage.setItem('harvest_last_local_update', new Date(data.updated_at).getTime().toString());
    },
  }
},
